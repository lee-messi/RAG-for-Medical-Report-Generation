...
def visualize_3d_by_organs(emb_3d, labels, pca):
    ...
    filename = 'embeddings_3d_by_organ.png'
...
def visualize_3d_by_labels(emb_3d, labels, pca):
    ...
    top_labels = [label for label, count in label_counts.most_common(20)]
    ...
    for i, label in enumerate(top_labels):
        ...
        ax.scatter(emb_3d[mask, 0], emb_3d[mask, 1], emb_3d[mask, 2],
                  c=[colors[i]], label=f"{label[:20]}... ({label_counts[label]})", 
                  alpha=0.7, s=15)
    
    ax.set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.1%})')
    ax.set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.1%})')
    ax.set_zlabel(f'PC3 ({pca.explained_variance_ratio_[2]:.1%})')
    ax.set_title('3D Embeddings by Label (Top 20)')
    ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=8)
    
    filename = 'embeddings_3d_by_label.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Saved: {filename}")
